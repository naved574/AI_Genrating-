...
import {
  LayoutDashboard, Users, Image as ImageIcon, Flag, FileText, BarChart3,
  DollarSign, Coins, Bell, Flag as FlagIcon, Cpu, CreditCard, Megaphone, Settings,
} from "lucide-react";
  ...
  return (
    <aside className="hidden lg:flex w-60 shrink-0 border-r border-sidebar-border bg-sidebar flex-col h-[calc(100vh-3.5rem)] sticky top-14 overflow-y-auto">
      <div className="p-4 space-y-1">
        <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-semibold px-2 mb-2">Admin</p>
        {items.map((it) => {
          const active = it.exact ? path === it.to : path === it.to || path.startsWith(it.to + "/");
          return (
            <Link key={it.to} to={it.to} className={`flex items-center gap-2.5 px-2.5 py-1.5 rounded-md text-sm transition-colors ${active ? "bg-accent/15 text-foreground font-medium" : "text-muted-foreground hover:text-foreground hover:bg-sidebar-accent"}`}>
