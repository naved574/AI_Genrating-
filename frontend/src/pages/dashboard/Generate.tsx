...
import { aspectRatios } from "@/data/mock";

export function Generate() {
  const s = useGeneration();
  const { loadHistory, loadModels } = s;
        ...
        <div className="p-5 space-y-7">
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full flex items-center gap-3 p-3 rounded-xl bg-background border border-border hover:border-accent/40 transition-colors"
            >
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {aspectRatios.map((a) => (
                <button
                  key={a}
                  onClick={() => s.setAspect(a as AspectRatio)}
              ...
              <button className="px-2.5 py-1.5 rounded-md text-[10px] font-medium border border-border text-muted-foreground hover:text-foreground">
                more
              </button>
            </div>
          </div>
          ...
          <div className="space-y-4 pt-2 border-t border-border">
              ...
              <div className="p-1 bg-background rounded-lg flex gap-1 border border-border">
                {(["steady", "turbo"] as const).map((sp) => (
                  <button
                    ...
                    className={`flex-1 py-1.5 text-[10px] font-medium rounded capitalize ${s.speed === sp ? "bg-muted text-foreground" : "text-muted-foreground"}`}
                  >
                    {sp}
                  </button>
                ))}
            ...
            <button onClick={s.toggleNegative} className="w-full flex items-center justify-between">
              ...
              <span
                className={`relative w-8 h-4 rounded-full transition-colors ${s.negativeEnabled ? "bg-accent" : "bg-muted"}`}
              >
                <span
                  className={`absolute top-0.5 size-3 bg-white rounded-full transition-transform ${s.negativeEnabled ? "translate-x-4" : "translate-x-0.5"}`}
                />
              </span>
            </button>
          </div>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2">
                {s.models.slice(0, 4).map((m) => (
                  ...
                  >
                    <div className="size-8 rounded-md bg-gradient-to-br from-accent to-primary-muted shrink-0" />
                    <div className="min-w-0 flex-1">
                      <p className="text-[11px] font-bold truncate">{m.name}</p>
                      <p className="text-[9px] text-muted-foreground truncate">
                        {m.cost_credits} credits · {m.provider}
                      </p>
                    </div>
                    ...
                    <div className="relative">
                      ...
                      <AnimatePresence>
                        {openMenu === r.id && (
                          ...
                          >
                            <button className="w-full px-3 py-2 text-left text-xs hover:bg-muted flex items-center gap-2">
                              <Download className="size-3.5" /> Download image
                            </button>
                            <button className="w-full px-3 py-2 text-left text-xs hover:bg-muted flex items-center gap-2">
                              <Info className="size-3.5" /> Take info
                            </button>
                            <button
                              onClick={() => {
                                s.remove(r.id);
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-[94%] max-w-3xl z-20">
          ...
          <div className="bg-card/95 border border-border backdrop-blur-xl rounded-2xl p-3 shadow-2xl">
            <div className="flex gap-2 items-start">
              <button
                title="Upload image"
                className="size-10 rounded-lg border border-border flex items-center justify-center hover:bg-muted text-muted-foreground shrink-0 mt-0.5"
              >
              ...
              <button
                disabled={s.loading}
                onClick={() => s.generate()}
                className="bg-accent text-accent-foreground px-4 h-10 rounded-lg font-bold text-xs uppercase tracking-wider shrink-0 mt-0.5 hover:opacity-90 disabled:opacity-60 animate-pulse-glow flex items-center gap-1.5"
              >
                <Sparkles className="size-3.5" /> {s.loading ? "Working" : "Gen img"}
              </button>
            </div>
            {s.negativeEnabled && (
